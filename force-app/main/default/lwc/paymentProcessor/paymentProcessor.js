...
export default class PaymentProcessor extends LightningElement {
	...
	@api sendPayment() {
		...
		this.stripe.makeCallout(data.auth, payload)
        .then((res) => {
			console.log(res);

			const fields = {};
			fields[STRIPE_TRANSACTION_ID_FIELD.fieldApiName] =
				this.paymentType === 'In Full' ? res.id : res.subscription;
        ...
        .catch(e => {
            console.log(e);
            this.showToast('Error', e.message.body, 'error')
        })
	}
